 ...
 *
 * const USER_ONE = 'UserNrOne';
 * const USER_TWO = 'WillBeAlwaysTheSecond';
 *
 * @Injectable()
 * class UserService {
 ...
 *     return this.http.get('myservices.de/api/users')
 *         .toPromise()
 ...
 * }
 *
 * describe('MockBackend UserService Example', () => {
 *   beforeEach(() => {
 ...
 *   });
 *
 *   it('getUsers() should query current service url', () => {
 *     this.userService.getUsers();
 *     expect(this.lastConnection).toBeDefined('no http service connection at all?');
 *     expect(this.lastConnection.request.url).toMatch(/api\/users$/, 'url invalid');
 *   });
 *
 *   it('getUsers() should return some users', fakeAsync(() => {
 *        let result: String[];
 *        this.userService.getUsers().then((users: String[]) => result = users);
 *        this.lastConnection.mockRespond(new Response(new ResponseOptions({
 *          body: JSON.stringify({data: [USER_ONE, USER_TWO]}),
 *        })));
 *        tick();
 *        expect(result.length).toEqual(2, 'should contain given amount of users');
 *        expect(result[0]).toEqual(USER_ONE, ' USER_ONE should be the first user');
 *        expect(result[1]).toEqual(USER_TWO, ' USER_TWO should be the second user');
 *      }));
 *
 ...
 *        this.userService.getUsers()
 *            .then((users: String[]) => result = users)
