...
Added supporting `UserService.searchUsers(term: string)` and test.
...
```
HttpClientInMemoryWebApiModule.forRoot(UserInMemDataService, { dataEncapsulation: true })
```
...
* The `HTTP GET` interceptor in example `user-data.service` shows how to create your own "cold" observable.
...
* Exports functions helpful for custom HTTP Method Interceptors
  ...
  * `setStatusText`
* Added `examples/user-data.service.ts` to show overrides (issue #44)
